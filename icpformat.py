...
"""
__all__ = ["ICP", "read", "summary", "data", "plot"]
...
class ICP(object):
    ...
    def asdata(self):
        ...
        d.vlabel = 'Counts'
        d.v = self.counts
        d.xlabel = self.columnnames[0].capitalize()
        d.x = self.column[self.columnnames[0]]
        if len(d.v.shape) > 1:
            d.ylabel = 'Pixel'
...
def _write_icp_frame(file, frame):
    """
    Write data for one PSD or area detector frame
    """
...
def write_icp_data(file, formats, columns, detector=None):
    ...
    If detector is provide, it will be an array of PSD or area detector frames.
    """
...
def replace_data(infilename, outfilename, columns, detector=None):
    """
    Copy infile to outfile replacing data with the contents of columns and
    with detector values for PSD and area detectors.
    """
    infile = open(infilename,'r')
    outfile = open(outfilename, 'w')

    # Copy everything up to and including the motor column
    while True:
...
def message(text):
    """Report an error from ICP load"""
    pass
def question(text):
    """Ask user a yes/no question during ICP load"""
    return True
...
def summary(filename):
    ...
    icp = ICP(filename)
    icp.summary() # Read the header only
    return icp

def data(filename):
    """Read an ICP file returning a generic Data object"""
    icp = ICP(filename)
    icp.read()
    return icp.asdata()
...
def plot(filename):
    """
    Read an ICP filename and make a plot
    """
    icp = ICP(filename)
    icp.read()
    icp.plot(show=True)
...
def demo():
    ...
    for file in sys.argv[1:]:
        fields = read(file)
        for k in sorted(fields.__dict__.keys()):
            print k,getattr(fields,k)
...
def test():
    """
    Read various icp files and check that they are read correctly
    """
    from . import utils
    import os
    filename = utils.example('bt5', 'sep 1/64D4A001.bt5')
    fields = read(filename)
    assert fields.wavelength == 2.38
    assert fields.column['qz'][-1] == 0.21
